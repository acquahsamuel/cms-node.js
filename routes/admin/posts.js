...
router.post("/create", (req, res) => {
    ...
    if (errors.length > 0) {
        ...
        newPost.save().then(savedPost => {
            req.flash(`success_message`, `Post ${
                savedPost.title
...
router.delete("/:_id", (req, res) => {
    Post.findOne({_id: req.params._id}).populate("comments").then(post => {
        fs.unlink(uploadDir + post.file, err => { // Deleting post with comments {Key Code}
